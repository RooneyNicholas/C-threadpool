...
MD5 no salt -- key="" -- $5$$3c2QQ0KjIU1OLtB29cl8Fplc2WN7X89bnoEjaR7tWu.
MD5 salt=112233 key=6834015 -- $5$112233$GOAACho98omkeguS3aGz19PO5QNBpvFCsjjGuR.l6YB
*/
...
typedef struct data {
    char *encrypted_hash;
    char test_string[9]; //8 max length + 1 null char
...
void decrypt_task(tpool_t pool, void* args) {
    ...
    if (strcmp(crypt_r(data->test_string, data->encrypted_hash, &cr_data), data->encrypted_hash) == 0) {
        printf("%s\n", data->test_string);
        exit(0);
    }

    int string_length = data->test_string_length + 1;
    for(int i = 0; i <= 9 && string_length <= 8; i++) {
        ...
        new_data->test_string[string_length] = '\0'; //ensure null terminated string
